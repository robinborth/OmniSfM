...
public:
    ...
    void runSfM(std::vector<Image>& images, std::vector<std::vector<Match>>& allMatches)
    {
        if (allMatches.size() > 0 && allMatches[0].size() >= 60)
        {
            ...
            if (estimateInitialPose(pts1, pts2, K, R, t))
            {
                ...
                cameraPoses.push_back(combineRotationAndTranslationIntoMatrix(R, t)); // Save initial pose
                // Triangulate points and store them
                points3D = triangulatePointsWithColor(pts1, pts2, R, t, K, colors1, colors2);
    ...
    const std::vector<ColoredPoint3f>& getPoints3D() const { return points3D; }
    const std::vector<Eigen::Matrix4f>& getCameraPoses() const { return cameraPoses; }
